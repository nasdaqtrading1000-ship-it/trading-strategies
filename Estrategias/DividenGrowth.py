...
def find_dividend_growth_candidates():
    ...
    for symbol in symbols:
        ...
        if df is None or df.empty:
            log_symbol_decision("Dividend Growth", symbol, "SIN DATOS", "Alpaca no devolvio velas")
            continue
        ...
        try:
            ...
            log_symbol_decision("Dividend Growth", symbol, "ERROR FMP", str(error))
            continue
        ...
        if result:
            accepted_count += 1
            log_symbol_decision("Dividend Growth", symbol, "OK", format_candidate(result))
            candidates.append(result)
        else:
            log_symbol_decision("Dividend Growth", symbol, "DESCARTADO", "No cumple dividendo, crecimiento, payout, liquidez o tendencia")
    ...
    log_strategy_summary("Dividend Growth", len(symbols), with_data_count, accepted_count, len(selected))
    return selected
