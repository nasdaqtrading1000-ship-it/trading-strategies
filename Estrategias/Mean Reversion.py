...
def find_mean_reversion_candidates():
    ...
    for symbol in symbols:
        ...
        if df is None or df.empty:
            log_symbol_decision("Mean Reversion", symbol, "SIN DATOS", "Alpaca no devolvio velas")
            continue
        ...
        if result:
            accepted_count += 1
            log_symbol_decision("Mean Reversion", symbol, "OK", format_candidate(result))
            candidates.append(result)
        else:
            log_symbol_decision("Mean Reversion", symbol, "DESCARTADO", "No cumple sobreventa, Bollinger, RSI, liquidez o estabilizacion")
    ...
    log_strategy_summary("Mean Reversion", len(symbols), with_data_count, accepted_count, len(selected))
    return selected
