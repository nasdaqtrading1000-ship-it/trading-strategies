...
def find_intraday_momentum_signals():
    ...
    for symbol in symbols:
        ...
        if df is None or df.empty:
            log_symbol_decision("Momentum Intradia", symbol, "SIN DATOS", "Alpaca no devolvio velas intradia")
            continue
        ...
        if result:
            accepted_count += 1
            log_symbol_decision("Momentum Intradia", symbol, "OK", format_signal(result))
            signals.append(result)
        else:
            log_symbol_decision("Momentum Intradia", symbol, "DESCARTADO", "No cumple momentum intradia, VWAP, volumen o tendencia")
    ...
    log_strategy_summary("Momentum Intradia", len(symbols), with_data_count, accepted_count, len(selected))
    return selected
