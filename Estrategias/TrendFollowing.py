...
def find_trend_following_candidates():
    ...
    for symbol in symbols:
        ...
        if df is None or df.empty:
            log_symbol_decision("Trend Following", symbol, "SIN DATOS", "Alpaca no devolvio velas")
            continue
        ...
        if result:
            accepted_count += 1
            log_symbol_decision("Trend Following", symbol, "OK", format_candidate(result))
            candidates.append(result)
        else:
            log_symbol_decision("Trend Following", symbol, "DESCARTADO", "No cumple tendencia de largo plazo, ruptura o liquidez")
    ...
    log_strategy_summary("Trend Following", len(symbols), with_data_count, accepted_count, len(selected))
    return selected
